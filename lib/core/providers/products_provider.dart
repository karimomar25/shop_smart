...
// 1- create products provider

class ProductsProvider with ChangeNotifier {
  // 2- create getter for our products model list
  List<ProductModel> get getProduct {
    return products;
  }

// create a func to show every product by its id in every view we want to show on it
  ProductModel? findByProductId(String productId) {
    // check first if our product not found return null to avoid an error
    if (products
        .where(
    ...
    // the implement of giving the id of the product to the productId variable of the func
...
// >> pass our provider to main.dart file to make the app access it 
// >>> go to our widget to pass the arguments of our product to the widget which it shown in the view